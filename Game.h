...
class Game {
	...
	int Scr_W = 800, Scr_H = 800;
	Camera2D WorldCam;
	std::vector<Behaviour*> Objects;
...
public:
	...
	float scl_top = 0, scl_left = 0, scl_bottom = 0, scl_right = 0;
	Point Bounding[4] = {};
	void run() {
		...
		Player p;
		PlayerMovement Pm = PlayerMovement(p);
		LevelDesigner Ld = LevelDesigner(p);
		...
		while (!WindowShouldClose()) {
			...
			rlPushMatrix(); // This locks the coords to local space only allowing objects to be transformed in he local space
			rlScalef(1, -1, 1);
